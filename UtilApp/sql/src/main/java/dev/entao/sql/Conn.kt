...
class Conn(val db: SQLiteDatabase) : Closeable {
		...
		if (pk != null) {
			if (pk.hasAnnotation<AutoInc>()) {
				if (!model.model.containsKey(pk.nameProp)) {
					assignId = true
				} else if ((model.model.get(pk.nameProp) as? YsonNum)?.data?.toInt() == 0) {
	...
	fun updateByKey(model: Model): Boolean {
		return this.updateByKey(model, kotlin.collections.emptyList())
	}
	...
	fun save(model: Model): Boolean {
		...
		if (hasPk) {
			return updateByKey(model)
		} else {
			return this.insert(model) != -1L
		}
	}
	...
	fun <T : Model> findAll(cls: KClass<T>, w: Where?, ob: OrderBy?): List<T> {
		return this.findAll<T>(cls) {
			where(w)
			this.orderBy(ob)
		}
	}
	...
	fun count(q: SQLQuery): Int {
		q.modelClassSet.forEach {
			TableCreatorX.check(db, it)
		}
	...
	fun query(q: SQLQuery): Cursor? {
		...
		val c = db.rawQuery(q.toSQL(), q.sqlArgs)
		return c
	}
