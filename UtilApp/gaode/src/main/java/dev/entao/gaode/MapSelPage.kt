...
import dev.entao.ui.viewcreator.relative
import dev.entao.ui.viewcreator.simpleListView
...
open class MapSelPage : TitlePage() {
    ...
    fun mapMarker(
        loc: LatLng,
        title: String?,
        bitmapDesc: BitmapDescriptor = BitmapDescriptorFactory.defaultMarker()
    ): Marker {
        val m = mapView.map.addMarker(
            MarkerOptions()
                .position(loc)
                .title(title)
                .anchor(0.5f, 0.5f)
